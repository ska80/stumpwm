...
(defmethod io-channel-handle ((channel request-channel) (event (eql :read)) &key)
  ...
  (loop
    ...
    do (read-byte in)
    while (listen in))
  (let ((events (with-lock ((request-channel-lock channel))
...
(defun push-event (fn)
  (with-lock ((request-channel-lock *request-channel*))
      ...
      (let ((ret (catch :top-level
        ...
        (cond ((and (consp ret)
              ...
              ((eq ret :hup-process)
               ...
               (apply 'execv (first (line-arguments-list)) (line-arguments-list)))
              ((eq ret :restart)
               (run-hook *restart-hook*))
              (t
               (run-hook *quit-hook*)
