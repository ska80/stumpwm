...
(defun bytes-to-string (data)
  ...
  #+lispworks
  (handler-bind ((ef:external-format-error
                   #'(lambda (c)
                       (declare (ignore c))
                       (return-from bytes-to-string
                         (make-string (length data) :initial-element #\?)))))
...
(defun utf8-to-string (octets)
  ...
  (handler-bind ((ef:external-format-error
                   #'(lambda (c)
                       (declare (ignore c))
                       (return-from utf8-to-string
                         (make-string (length octets) :initial-element #\?)))))
...
(defun string-to-octets (string)
  ...
  #+lispworks (ef:encode-lisp-string string :utf-8))
   ...
   (let ((new (make-string stringlen)))
     ...
     (setq string new)))
       ...
       (setf (subseq string string-filled)
             (octets-to-string (subseq buf 0 pos)))
